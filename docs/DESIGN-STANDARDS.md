...
- **Botón de borrar por fila**: `×` discreto al final de la fila, oculto (`visibility:hidden`) cuando borrar dejaría la lista vacía. Reemplazó al swipe: **el swipe se descartó** porque es invisible, no se descubre y en filas llenas de inputs no se dispara nunca.
