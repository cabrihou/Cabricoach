...
def revisar_iconos(limpio, js):
    ...
    # ic() consulta SOLID ANTES que ICONS: un nombre que solo esta en SOLID
    # igual se dibuja bien. Sin esto se reportaban 4 falsos positivos.
    ms = re.search(r"const SOLID\s*=\s*\{(.*?)\n\}", js, re.S)
    if ms:
        disponibles |= set(re.findall(r"^\s*([A-Za-z_$][\w$]*)\s*:", ms.group(1), re.M))
